...
class CartManager {
    ...
    // if array's #carts is 0, retruns 1, else sum 1 to max number.
    autoId = () => {
        if(this.#carts.length === 0) {
            return 1
        } else {
            // The spread operator takes every element one to one to pass them in the map() function
            const maxId = Math.max(...this.#carts.map(cart => cart.id))
            return maxId + 1
        }
    }

    // when I read the file and it is empty, or a parsing error returns an empty array
    async getCarts() {
        try {
            // Here, I'm passing a path and utf-8 format file.
    ...
    async createCart(products = []) {
        ...
        // Cheking the cartIdValidate() function
